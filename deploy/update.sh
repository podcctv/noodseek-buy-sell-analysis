...
if [[ -x "${INSTALL_DIR}/deploy/install.sh" ]]; then
  REPO_SLUG="${REPO_SLUG}" BRANCH="${BRANCH}" INSTALL_DIR="${INSTALL_DIR}" bash "${INSTALL_DIR}/deploy/install.sh"
  exit 0
fi
...
chmod +x "${TMP_SCRIPT}"
SCRIPT_PATH="${TMP_SCRIPT}" REPO_SLUG="${REPO_SLUG}" BRANCH="${BRANCH}" INSTALL_DIR="${INSTALL_DIR}" bash "${TMP_SCRIPT}"
rm -f "${TMP_SCRIPT}"
